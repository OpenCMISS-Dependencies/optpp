...
//   InvertedMatrix i;
   ...
   virtual ReturnMatrix sum_square_columns() const;
   virtual ReturnMatrix sum_square_rows() const;
...
#endif
   MatrixType Type() const;
   SymmetricMatrix(const SymmetricMatrix& gm) : GeneralMatrix()
      { GetMatrix(&gm); }
   Real SumSquare() const;
   Real SumAbsoluteValue() const;
   Real Sum() const;
...
public:
   ...
   UpperTriangularMatrix(const UpperTriangularMatrix& gm) : GeneralMatrix()
      { GetMatrix(&gm); }
   void operator=(Real f) { GeneralMatrix::operator=(f); }
   Real& operator()(int, int);                  // access element
   Real& element(int, int);                     // access element
...
class CroutMatrix : public GeneralMatrix        // for LU decomposition
{
   ...
   bool sing;
   void ludcmp();
   void operator=(const CroutMatrix&) {}     // not allowed
public:
...
public:
   ...
   MatrixType Type() const;
   UpperBandMatrix(const UpperBandMatrix& gm) : BandMatrix()
      { GetMatrix(&gm); }
...
public:
   ...
   MatrixType Type() const;
   LowerBandMatrix(const LowerBandMatrix& gm) : BandMatrix()
      { GetMatrix(&gm); }
   GeneralMatrix* MakeSolver() { return this; }
   void Solver(MatrixColX&, const MatrixColX&);
   LogAndSign LogDeterminant() const;
...
#endif
   MatrixType Type() const;
   SymmetricBandMatrix(const SymmetricBandMatrix& gm) : GeneralMatrix()
      { GetMatrix(&gm); }
   GeneralMatrix* MakeSolver();
   Real SumSquare() const;
   Real SumAbsoluteValue() const;
...
// for LU decomposition of band matrix
{
   ...
   void operator=(const BandLUMatrix&) {}     // no allowed
public:
...
public:
   ...
   IdentityMatrix(const IdentityMatrix& gm) : GeneralMatrix() 
      { GetMatrix(&gm); }
...
public:
   GenericMatrix() : gm(0) {}
   GenericMatrix(const BaseMatrix& bm) : BaseMatrix()
...
public:
   ...
   ReturnMatrix(const GeneralMatrix* gmx)
      : BaseMatrix(), gm((GeneralMatrix*&)gmx) {}
